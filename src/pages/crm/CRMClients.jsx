...
import { Search, Plus, Trash2, TrendingUp, TrendingDown, X, Users } from 'lucide-react'
...
const COLORS = ['#111','#059669','#4A6CF7','#F59E0B','#EC4899','#EF4444','#06B6D4','#8B5CF6']
...
export default function CRMClients() {
  ...
  const isAdmin = user.email === 'admin@aurasocials.com'
  const storageKey = `aura_clients_${user.email}`

  const getInitialClients = () => {
    if (isAdmin) return DEMO_CLIENTS
  ...
  const saveClients = (updated) => {
    setClients(updated)
    if (!isAdmin) localStorage.setItem(storageKey, JSON.stringify(updated))
  }
  ...
  const toggleNet = (n) => setForm(f => ({
    ...f, networks: f.networks.includes(n) ? f.networks.filter(x=>x!==n) : [...f.networks, n]
  }))

  const addClient = () => {
    if (!form.name.trim()) return
    const newClient = {
      id: Date.now(), name: form.name, sector: form.sector,
      networks: form.networks, posts: 0, reach: '0', engagement: '0%',
      status: form.status, color: COLORS[Math.floor(Math.random()*COLORS.length)]
    }
  ...
  const EmptyState = () => (
    <div style={{textAlign:'center',padding:'80px 20px'}}>
      <div style={{width:80,height:80,background:'#EEF1FB',borderRadius:24,display:'flex',alignItems:'center',justifyContent:'center',margin:'0 auto 20px'}}>
        <Users size={36} style={{color:'#4A6CF7'}}/>
      </div>
      <h3 style={{fontSize:20,fontWeight:700,color:'#111827',marginBottom:8}}>Aucun client pour le moment</h3>
      <p style={{fontSize:14,color:'#6B7280',marginBottom:28,maxWidth:340,margin:'0 auto 28px'}}>
        Commencez par ajouter votre premier client pour gérer ses réseaux sociaux avec AuraCRM.
      </p>
      <button className="crm-btn" onClick={() => { setForm({name:'',sector:'Mode & Sport',networks:['IG'],status:'Actif'}); setModal(true) }}>
        <Plus size={16}/> Ajouter mon premier client
      </button>
    </div>
  )
  ...
  return (
    <div>
          ...
          <div className="modal-box">
            <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:24}}>
              <div>
                ...
                <div className="modal-sub">Ajoutez un client à votre portfolio AuraCRM</div>
              </div>
            ...
            <div className="modal-field">
              ...
              <input className="modal-input" placeholder="Ex : Zara Paris" autoFocus
            ...
            <div className="modal-field">
              <label className="modal-label">Réseaux sociaux</label>
              <div className="modal-nets">
                {ALL_NETS.map(n => (
                  <button key={n} className={`modal-net ${form.networks.includes(n)?'on':''}`} onClick={() => toggleNet(n)}>
            ...
            <div className="modal-btns">
              ...
              <button className="modal-submit" onClick={addClient} disabled={!form.name.trim()}>✓ &nbsp;Ajouter</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:24}}>
        <div>
          <h2 style={{fontSize:22,fontWeight:800,color:'#111827'}}>
            {isAdmin ? 'Gestion clients (démo)' : 'Mes clients'}
          </h2>
          <p style={{fontSize:14,color:'#9CA3AF',marginTop:4}}>
            {clients.length} client{clients.length!==1?'s':''} · {clients.filter(c=>c.status==='Actif').length} actifs
            {isAdmin && <span style={{color:'#4A6CF7',fontWeight:600}}> · Compte administrateur</span>}
          </p>
        </div>
        <button className="crm-btn" onClick={() => { setForm({name:'',sector:'Mode & Sport',networks:['IG'],status:'Actif'}); setModal(true) }}>
      ...
      <div className="crm-kpi-grid" style={{marginBottom:24}}>
        {[
          [String(clients.length),'Total clients','Votre portfolio','#4A6CF7'],
          [String(clients.filter(c=>c.status==='Actif').length),'Clients actifs',clients.length?'↑ En hausse':'Ajoutez votre premier !','#059669'],
          [clients.reduce((s,c)=>s+c.posts,0).toString(),'Posts publiés','Ce mois','#4A6CF7'],
          [clients.length?'2.4M':'0','Impressions',clients.length?'↑ +8%':'À construire','#059669'],
        ].map(([v,l,d,c]) => (
          ...
          <>
            <div className="crm-toolbar">
              <div className="crm-search-wrap">
                ...
                <input className="crm-search" placeholder="Rechercher..." value={search} onChange={e => setSearch(e.target.value)}/>
              </div>
              <div className="crm-filter-group">
                {['Tous','Actif','Inactif'].map(f => (
                  <button key={f} onClick={() => setFilter(f)} className={`crm-filter-btn ${filter===f?'active':''}`}>{f}</button>
                ))}
              </div>
            ...
            <div className="crm-table-wrap">
              <table className="crm-table">
                <thead><tr>{['Client','Secteur','Réseaux','Posts','Reach','Engagement','Statut','Action'].map(h=><th key={h}>{h}</th>)}</tr></thead>
                <tbody>
                  {filtered.map(c => (
                    <tr key={c.id}>
                      <td>
                      ...
                      <td><div style={{display:'flex',gap:4}}>{c.networks.map(n=><span key={n} className="crm-net-badge" style={{background:NET_C[n]||'#888'}}>{n}</span>)}</div></td>
                      <td style={{fontSize:13,fontWeight:700,color:'#111827'}}>{c.posts}</td>
                      <td style={{fontSize:13,color:'#374151'}}>{c.reach}</td>
                      <td><span style={{fontSize:13,fontWeight:700,color:parseFloat(c.engagement)>=5?'#059669':'#374151',display:'flex',alignItems:'center',gap:4}}>{parseFloat(c.engagement)>=4?<TrendingUp size={13}/>:<TrendingDown size={13}/>}{c.engagement}</span></td>
                      <td><span className={`crm-status-badge ${c.status==='Actif'?'crm-status-actif':'crm-status-inactif'}`}>{c.status}</span></td>
                      <td><button onClick={() => deleteClient(c.id,c.name)} style={{background:'none',border:'1.5px solid #FEE2E2',borderRadius:8,padding:'6px 8px',cursor:'pointer',color:'#EF4444'}}><Trash2 size={14}/></button></td>
                    </tr>
                  ))}
                </tbody>
              </table>
