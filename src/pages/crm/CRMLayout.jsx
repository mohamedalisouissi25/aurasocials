...
import { LayoutDashboard, Users, Calendar, Brain, BarChart3, Bell, Settings, LogOut, ChevronLeft, ChevronRight } from 'lucide-react'
      ...
      <div className="crm-kpi-grid" style={{marginBottom:24}}>
        {[['2.4M','Impressions totales','+8%'],['29.3K','Abonnés totaux','+3.2%'],['175K','Reach moyen','+5.1%'],['79','Posts publiés','ce mois']].map(([v,l,d])=>(
          <div key={l} className="crm-kpi-card">
            ...
            <div className="crm-kpi-label" style={{textTransform:'none',fontSize:13,color:'#6B7280',fontWeight:400,letterSpacing:0}}>{l}</div>
            <div className="crm-kpi-delta" style={{color:'#059669'}}>{d}</div>
          </div>
        ))}
      </div>
...
export default function CRMLayout() {
  ...
  const currentPage = NAV.find(n => n.path === loc.pathname || (n.path !== '/crm' && loc.pathname.startsWith(n.path)))
      ...
      <aside className={`crm-sidebar ${collapsed ? 'closed' : 'open'}`}>
        ...
        <nav className="crm-nav">
          {NAV.map(({path,label,icon:Icon}) => {
            const active = loc.pathname === path || (path !== '/crm' && loc.pathname.startsWith(path))
            return (
              <button key={path} onClick={() => nav(path)} className={`crm-nav-item ${active ? 'active' : ''}`}>
        ...
        <div className="crm-nav-bottom">
          <button className="crm-nav-item" style={{width:'100%'}}>
